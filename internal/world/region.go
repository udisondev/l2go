...
// (как Wire: конкурентный вызов при живом Run — вне контракта, guard —
// best-effort); применение — первым шагом региона (drain контрольного ящика
// первым), сид разворота — (regionID, tick шага). cfg среза — часть письма:
// реплей воспроизводит разворот из лога порций (логи для реплея пишутся с
// LogPayloads=true).
...
func (r *Region) recovered(p any) {
	...
	r.advWindow = false
	// Письмо разворота однократно: паника после его изъятия уничтожила пачку
	// классово — население не развёрнуто навсегда, живой регион без NPC
	// молчать не должен (F27 реестра P3.10). Детектор — по факту: письмо было
	// в дренированной волне, а ни одного NPC-жителя нет (частичная популяция
	// окном phaseEffects не различается — ограничение, счётчики в Dump).
	if r.static != nil && !npcAmongResidents(r.residents) {
