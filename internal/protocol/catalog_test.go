...
func TestConstantsMatchCatalog(t *testing.T) {
	consts := []struct {
		...
		{"VALIDATE_POSITION", validatePosition, "C→GameServer", ""},
		{"CHAR_MOVE_TO_LOCATION", charMoveToLocation, "GameServer→C", ""},
		{"CHAR_INFO", charInfo, "GameServer→C", ""},
		{"USER_INFO", userInfo, "GameServer→C", ""},
		{"DELETE_OBJECT", deleteObject, "GameServer→C", ""},
		...
		{"STOP_MOVE", stopMove, "GameServer→C", ""},
		{"CREATURE_SAY", creatureSay, "GameServer→C", ""},
		{"SKILL_LIST", skillList, "GameServer→C", ""},
		{"VALIDATE_LOCATION", validateLocation, "GameServer→C", ""},
