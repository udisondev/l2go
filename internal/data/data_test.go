...
// (stats/items, stats/npcs, stats/skills, spawns, zones — пустые), чтобы
// MapFS-фикстуры отдельных категорий не падали фатально на чужом отсутствующем
// каталоге.
func catFS(files map[string]*fstest.MapFile) fstest.MapFS {
	out := fstest.MapFS{
		"stats/items/.keep":  &fstest.MapFile{},
		"stats/npcs/.keep":   &fstest.MapFile{},
		"stats/skills/.keep": &fstest.MapFile{},
...
func TestLoadCounters(t *testing.T) {
	...
	if rep.EmptyValues != 2 {
		t.Errorf("EmptyValues = %d; want 2 (пустой icon у 9005 + пустой feed у 7005)", rep.EmptyValues)
	}
	if rep.DupKeys != 2 {
