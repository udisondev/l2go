...
func TestDoublePutAlias(t *testing.T) {
	// Контракт: двойной Put запрещён; пул не детектирует дубликат и молча
	// выдаст один массив двоим. Фальсифицируемая часть — оба Put приняты
	// (счётчик), перекрытие владений — документация следствия.
	p := &Pool{}
	b := p.Get(128)
	p.Put(b)
	p.Put(b) // нарушение контракта
	if got := p.Stats().Puts; got != 2 {
		t.Errorf("Stats().Puts = %d; want 2 (двойной Put принят пулом)", got)
	}
	a1 := p.Get(100)
	a2 := p.Get(100)
	if &a1[0] == &a2[0] {
		t.Logf("документация следствия: двойной Put даёт перекрытие владений (контракт на вызывающем)")
	}
}
